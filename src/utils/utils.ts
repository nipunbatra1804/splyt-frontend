...
import { Position, OfficeLocation } from "src/types/types";
...
function distance(lat1:number, lon1:number, lat2:number, lon2:number, unit="K"): number {
...
export const getClosestOrDefaultOffice = (position:Position | null) : OfficeLocation => {
    if(position) {
        return getClosestOffice(position);
    }
